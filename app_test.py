...
model_name = "meta-llama/Llama-3.2-3B-Instruct" #microsoft/Phi-3.5-mini-instruct
sample_frac = 0.0006
        ...
        try:
            ...
            with st.spinner("Generating Visual Suggestions..."):
                future = executor.submit(
                    ...
                    user_prompt_2, {"visuals_list": visuals_list}
                )
                response_2, _ = future.result(timeout=None)  # Timeout after 60 sec
            ...
            if confirm_visuals == "Yes":
                ...
                with st.spinner("Running Position Analysis..."):
                    future = executor.submit(
                        ...
                        api_key, model_name, file_path, sheet_names[0], sample_frac,
                        f"{user_prompt_3} ",
                        {"visuals_list": visuals_list, "response_2": response_2}
                    )
                    
                    final_response, _ = future.result(timeout=None)  # Timeout after 2 min
                    visual_location=final_response
                ...
                with st.spinner("Running Visual Data Mapping..."):
                    future = executor.submit(
                        ...
                        api_key, model_name, file_path, sheet_names[0], sample_frac,
                        f"{user_prompt_6} ",
                        {"visual_parameters_list": visual_parameters_list, "response_2": response_2}
                    )
                    
                    final_response, _ = future.result(timeout=None)  # Timeout after 2 min
                    VisualDatatype=final_response
                ...
                time.sleep(2)
                Finale_backend.psuedo_main(Datatype,visual_location,VisualDatatype,file_path)
                time.sleep(3)
